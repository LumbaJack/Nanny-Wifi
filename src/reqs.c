...
BAD_REQUEST_ERROR:
	...
	if (addurl != NULL) {
		...
		connptr->WL = FALSE;
	        ...
	        log_message (LOG_NOTICE, "Return from filter %i", ret);
                ...
                if (ret == 1) {
                        ...
                        write_message (connptr->client_fd,"HTTP/1.1 301 Redirect\r\nLocation: http://safe.nanny.io/\r\nCache-Control: no-cache, no-store, must-revalidate\r\nPragma: no-cache\r\nExpires: 0\r\n\r\n");
                        goto fail;
                } else if (ret == 0) {
			connptr->WL = TRUE;
		}
...
static char *get_content_type (hashmap_t hashofheaders)
{
        ssize_t len;
        char *data;
...
process_client_headers (struct conn_s *connptr, hashmap_t hashofheaders)
{
	...
	/* Disabled  compress */
	if (hashmap_search (hashofheaders,"Accept-Encoding") > 0)  {
...
retry:
	...
	connptr->content_type = get_content_type (hashofheaders);
	memset(connptr->content_txt, 0, sizeof(connptr->content_txt));
...
static void relay_connection (struct conn_s *connptr)
{
	...
	char blockcode[] = "</script><script>window.location.href='http://safe.nanny.io/';</script><noscript><meta http-equiv='refresh' content='0; url=http://safe.nanny.io/' /></noscript>";
		...
		if ((connptr->content_type != NULL)
                    && (strstr(connptr->content_type, "html") != NULL)
                    && (connptr->WL == FALSE)) {
			append_txt (connptr->sbuffer, (char*) connptr->content_txt);
		}
	...
	if ((connptr->content_type != NULL)
		&& (strstr(connptr->content_type, "html") != NULL)
                && (connptr->WL == FALSE)) {
		webac = dllExecute (connptr->content_txt);
                log_message(LOG_INFO,
                            "Return from webfilter: %i %i", webac, connptr->WL);
		if (webac == 1) {
			/* add_to_buffer (connptr->sbuffer, (unsigned char *) blockcode, strlen(blockcode));  */
			 ret = write_message (connptr->client_fd, (char *) blockcode);
			return;
		}
	}
...
void append_txt(struct buffer_s * buffptr , char *data) {
	...
	if (strlen(data) < 51199) {
		if (strlen(data) + strlen((char*)line->string) <= 51199) {
			strncat(data, (char*)line->string, strlen((char*)line->string));
		} else {
			strncat(data, (char*)line->string, 51199 - strlen(data));
		}
	}
}
