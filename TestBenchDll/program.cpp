...
int main ( int argc, char* argv[] )
{
    ...
    Program program;
    HtmlRetreive htmlObj;
    
    std::string siteContents = htmlObj.UrlRetrieval(argv[1]);
    if(siteContents.size() > 50000){
        siteContents = siteContents.substr(0, 50000);
    };
    
    begin = clock();
    rvalue = program.execute(siteContents);
    end = clock();
    
    double elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;    
    std::cout<<"Return code: "<<rvalue<<std::endl;
    std::cout<<"Total elapsed time in seconds: "<<elapsed_secs <<std::endl;
